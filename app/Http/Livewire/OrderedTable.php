...
class OrderedTable extends Component
{
    ...
    public function mount(){
        ...
        $this->tasks = Task::with('user:id,name,sector_id,role_id')->whereNull('project_id')->whereHas('creator', function ($query) {
            $query->where('sector_id', auth()->user()->sector_id); })->where('status', '<>', "Выполнено")
                        ->latest()->get();
            ...
            if($this->status == "Empty"){
                ...
                if($this->status == "Просроченный"){
                    $this->tasks = Task::with(['user:id,name,sector_id,role_id'])->whereNull('project_id')->whereHas('creator', function ($query) {
                                    ...
                                    ->where('overdue', 1)->latest()->get();
                }else{
                    $this->tasks = Task::with(['user:id,name,sector_id,role_id'])->whereHas('creator', function ($query) {
                                    $query->where('sector_id', auth()->user()->sector_id); })
